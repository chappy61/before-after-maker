...
const isRealTouchDevice = () => (navigator.maxTouchPoints || 0) > 0;
const isRealTouchInput = (e) =>
  e.pointerType === "touch" && isRealTouchDevice();

// どこでキャプチャするか：基本は split に寄せると安定
const captureOn = (el, pointerId) => {
  try {
...
split?.addEventListener(
  ...
  (e) => {
    ...
    if (pointers.size === 0) {
      ...
      activeIndex = Number(hitPane.dataset.index);
      highlightActive();
      syncSlidersToActive();
    } else {
...
split?.addEventListener(
  ...
  (e) => {
    ...
    if (!gesture || gesture.type !== "pinch") {
      const pts0 = [...pointers.values()];
      const a0 = pts0[pts0.length - 2];
      const b0 = pts0[pts0.length - 1];

      gesture = {
        type: "pinch",
        startDist: dist(a0, b0),
        startAng: angle(a0, b0),
        startCenter: center(a0, b0),
        baseScale: ed.scale ?? 1,
        baseRotate: ed.rotate ?? 0,
        baseX: ed.x || 0,
        baseY: ed.y || 0,
